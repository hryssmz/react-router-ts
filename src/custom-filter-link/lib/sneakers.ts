// custom-filter-link/lib/sneakers.ts
...
export const sneakers: readonly Sneaker[] = [
  {
    id: "1",
    colorway: "Pine Green",
    imageUrl:
...
export function filterByBrand(brand: string) {
  return sneakers.filter(
    sneaker => sneaker.brand.toLocaleLowerCase() === brand.toLocaleLowerCase()
  );
}
